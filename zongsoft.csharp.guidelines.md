...
| 控制语句 | 关键字与左括号之间不加空格；同层级的相邻独立控制结构之间留一个空行，连续简单无花括号 `if`、`for`、`foreach`、`while` 例外 |
...
- 同一层级的相邻独立控制语句或显式语句块之间留一个空行，避免逻辑段粘连；连续的简单 `if`、`for`、`foreach`、`while` 语句（可混合排列）均无花括号、语句体为单条非控制语句且 `if` 不含 `else` 时，可以不插入空行。注释行不能代替空行。`if`／`else if`／`else`、`try`／`catch`／`finally` 和 `do`／`while` 属于同一结构，其关联子句之间不因此插入空行；控制语句与其嵌套语句体也不拆开。
