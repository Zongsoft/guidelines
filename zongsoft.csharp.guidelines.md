...
| 成员布局 | 文件内方法与属性合计超过 9 个时必须按职责使用中文 `#region`，不创建空分段并尽量让分段名保持 4 个汉字 |
...
- 一个代码文件内的方法与属性合计超过 9 个（即至少 10 个）时，必须使用成对的 `#region`／`#endregion` 按职责组织这些成员。按当前文件统计，包含嵌套类型及同文件的其他类型；索引器计作属性，构造函数、访问器、局部函数及运算符不计入，`partial` 的其他文件不合并计数。检测边界见 [ZS2004](RULES.zh-Hans.md#zs2004)。
