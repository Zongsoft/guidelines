...
| 控制语句 | 关键字与左括号之间不加空格；同层级的相邻独立控制结构之间留一个空行 |
...
- 移除未使用的引用，但允许保留普通的 `using System;`。这一例外不包括 `System.*`、别名、`using static` 或 `global using`。正式项目慎用 `global using`，优先在各文件中显式导入所需命名空间，保持依赖清晰；隐式引用遵循项目统一配置。单元测试项目可在 `Global.cs` 中集中导入通用的测试框架命名空间。
...
- 类型和多行语句块的左右花括号各占一行，采用 Allman 风格。自动属性、短表达式成员和空构造函数可以单行书写。整体简短清晰且每个子句仅含一条简单语句时，`try`、`catch`、`finally` 的块也可以各自单行书写；任一子句包含多条语句、嵌套控制流或跨行表达式时，使用常规多行块。
- `#if`、`#elif`、`#else`、`#endif` 等条件编译指令可以顶格，也可以与所在代码保持相同缩进层次；不因嵌套条件额外增加缩进。
- 控制关键字与括号之间不加空格，如 `if(value == null)`、`foreach(var item in items)`、`using(var stream = ...)`；声明和调用的括号内侧不加空格。
