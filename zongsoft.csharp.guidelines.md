...
- 类型和多行语句块的左右花括号各占一行，采用 Allman 风格。自动属性、短表达式成员和空构造函数可以单行书写；构造函数初始化器跨行时，空的 `{ }` 也可在一个空格后紧接 `this(...)`／`base(...)`。方法、局部函数及 lambda／匿名方法的单行块体最多包含两条简单语句，不包含嵌套控制流或跨行表达式。`try`、`catch`、`finally` 子句为空或仅含一条简单语句时，可以单行书写，也可把相邻简短子句放在同一行；允许单行与多行子句混用。含多条语句、嵌套控制流或跨行表达式的子句仍使用常规多行块。紧凑布局不改变异常处理的语义要求。
...
- 二元运算符两侧、逗号之后使用一个空格；一元运算符和成员访问运算符不加空格。变量名之前、声明初始化的 `=` 之前可以增加空白以纵向对齐；初始化、赋值及返回的二元或条件表达式续行可以对齐表达式开头；条件赋值的续行也可对齐赋值号 `=`。
...
- 同一层级的相邻独立控制语句或显式语句块之间留一个空行，避免逻辑段粘连；连续的简单 `if` 语句均无花括号、无 `else` 且语句体不含嵌套控制结构时，可以不插入空行。注释行不能代替空行。`if`／`else if`／`else`、`try`／`catch`／`finally` 和 `do`／`while` 属于同一结构，其关联子句之间不因此插入空行；控制语句与其嵌套语句体也不拆开。
...
- 测试项目（`IsTestProject=true`）中的异常消息、断言文本及其他测试场景文本可以直接使用字符串字面量，不要求本地化；此例外不适用于被测试的生产项目。
- 面向用户、操作人员或调用方开发者的自然语言文本应尽量本地化，包括提示信息、异常消息、交互文本、验证反馈及可读的诊断信息。将可翻译文本集中放入 `.resx` 资源，避免散落在代码中。
...
- 固定字符集、ANSI 终端控制序列及不含自然语言的纯字符图案不要求本地化。`MemoryExtensions.AsSpan`／`AsMemory` 的字符串视图转换本身也不表示需要翻译；转换后的文本若用于界面或消息，仍按实际用途审查。自动豁免的识别边界见 [ZSS1303](RULES.zh-Hans.md#zss1303)。
