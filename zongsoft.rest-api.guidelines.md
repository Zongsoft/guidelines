...
如果抽象子资源的方式受限于技术实现，也可以采取基于 `POST` 方法加 `资源/动词` 的组合方式，仍以上面操作为例：
...
公共参数指通过 _URL_ 中 _`查询字符串`_ 所指定的参数。

#### 分页

分页参数名为：`page`，支持三种分页模式。

> 💡 如果未指定该参数，则默认返回首页数据，页大小为系统默认值。
...
1. 禁用分页
	...
	> 🚨 注意：禁用分页可能会因为查询数据量过大导致系统出错，默认应关闭该项功能。

2. 页号模式
	> 用法：`page={index}` 或 `page={index}|{size}`
	> - `index` **必须项**，表示 _页号_（从 `1` 开始）；
	> - `size` _可选项_，表示 _页大小_，即每页的记录数，如果未指定该参数则表示采用系统默认值。

	> 💡 如果同时指定了 _页号_ 与 _页大小_，则它们之间使用 `|` 符分隔。

3. 偏移模式
	> 用法：`page={count}@{offset}`
	> - `count` **必须项**，表示获取的 _记录数_；
	> - `offset` **必须项**，表示获取的 _偏移量_，即跳过的记录数。

	> 💡 _记录数_ 与 _偏移量_ 之间使用 `@` 符分隔。

#### 排序

排序参数名为：`sort`，支持多个排序字段及独立的升降序。

> 🚨 注意：由于排序对数据库查询效能有严重影响，因此不是所有字段都支持排序，具体支持的排序字段需咨询服务提供者。
