...
关于资源名是单数还是复数的问题，应遵循资源所蕴含的数量语义，在实际技术实现中如果不能确保每个资源的准确含义，那应该尽量统一单复数命名（切勿单复数形式混搭）。
...
通常 GET 和 DELETE 方法应支持过滤条件，其条件字段名与字段值中间用 `:` 分隔，多个条件项用 `+` 号连接。其字段值除支持常规的数字、字符、布尔、日期时间类型外，还支持 _集合_ 与 _区间_ 两种类型。
